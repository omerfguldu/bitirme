...
class KarsiProfilGaleriGrid extends StatefulWidget {
  ...
  bool benimmi;
  KarsiProfilGaleriGrid({this.imageList,this.galeriBaslik,this.userid,this.imageListId,this.card, @required this.benimmi});

  @override
  _KarsiProfilGaleriGridState createState() => _KarsiProfilGaleriGridState();
}
...
class _KarsiProfilGaleriGridState extends State<KarsiProfilGaleriGrid> {
  ...
  Future<void> choiceAction(String choice) async {
    if(choice == GaleriSecim.ekle){
      ...
      await  FirebaseFirestore.instance.collection("users").doc(widget.userid).collection("albumlerim").doc(widget.imageListId.toString()).delete();
      for(var item in widget.imageList){
        var fileUrl = Uri.decodeFull(Path.basename(item.toString())).replaceAll(new RegExp(r'(\?alt).*'), '');

        final Reference firebaseStorageRef = FirebaseStorage.instance.ref(widget.userid).child("usersAlbum").child(fileUrl);
        String firebaseBucket = FirebaseStorage.instance.ref(item.toString()).bucket;
        print("firebase storage url: "+"gs://"+firebaseBucket+"/"+widget.userid+"/usersAlbum/"+firebaseStorageRef.name);

        Reference photoRef6 = await   FirebaseStorage.instance.ref().storage.refFromURL("gs://"+firebaseBucket+"/"+widget.userid+"/usersAlbum/$etadi/"+firebaseStorageRef.name);
        print("Fotograf yolu:  "+"gs://"+firebaseBucket+"/"+widget.userid+"/usersAlbum/$etadi/"+firebaseStorageRef.name);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar:  AppBar(
        ...
        actions:  <Widget>[
          widget.benimmi ?   PopupMenuButton<String>(
      ...
      body: ClipRRect(
        ...
        child: Container(
          ...
          child: albumyukleniyor ? Center(child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            crossAxisAlignment: CrossAxisAlignment.center,
            children: [
              Text("Fotoğraflarınız Yükleniyor ....",style: TextStyle(fontSize: 20.0.spByWidth,fontWeight: FontWeight.bold),),
              SizedBox(height: 30.0.h,),
              CircularProgressIndicator(),
            ],
          ),) : albumsiliniyor ?  Center(child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            crossAxisAlignment: CrossAxisAlignment.center,
            children: [
              Text("Fotoğraflarınız Siliniyor ....",style: TextStyle(fontSize: 20.0.spByWidth,fontWeight: FontWeight.bold),),
              SizedBox(height: 30.0.h,),
              CircularProgressIndicator(),
            ],
          ),) :
                ...
                return Padding(
                  ...
                  child: InkWell(
                    onTap: (){
                      ...
                      Navigator.of(context).push(MaterialPageRoute(builder: (_) => ImageViewGallery(
                        imageList: imagelist,
  ...
  Future<void> loadAssets() async {
    ...
    DocumentSnapshot snapshot= await  FirebaseFirestore.instance.collection("users").doc(widget.userid).collection("albumlerim").doc(widget.imageListId.toString()).get();
  ...
  Future<String> uploadAlbumImageFile(File pickedImage,String index) async {
    ...
    try {
      ...
      String downloadURL = await firebase_storage.FirebaseStorage.instance.ref(userId+'/usersAlbum/$etadi/$index').getDownloadURL();
      return downloadURL;
