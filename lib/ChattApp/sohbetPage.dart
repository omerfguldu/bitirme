...
class _SohbetPageState extends State<SohbetPage> {
  ...
  Widget _buildMesajListesi() {
    return Consumer<ChatViewModel>(builder: (context, chatModel, child) {
      FirebaseFirestore.instance
          .collection("users")
          .doc(chatModel.currentUser.userId)
          .collection("sohbetler")
          .doc(chatModel.sohbetEdilenUser.userId)
          .update({
        "konusma_goruldu": true,
      });
      return Expanded(
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            if (chatModel.hasMoreLoading && chatModel.mesajlarListesi.length == index) {
              ...
              return _SohbetkonusmaBalonuOlustur(
                  chatModel.mesajlarListesi[index], chatModel.mesajlarListesi[0].goruldumu);
          },
  ...
  Widget _SohbetkonusmaBalonuOlustur(Mesaj oankiMesaj, bool sonmesajgorunme) {
    ...
    final icon = sonmesajgorunme
        ? Icon(
            Icons.done_all,
            color: Colors.blue,
            size: 13.0.h,
          )
        : oankiMesaj.goruldumu
            ? Icon(
                Icons.done_all,
                color: Colors.blue,
                size: 13.0.h,
    ...
    var messageDate = DateTime.fromMillisecondsSinceEpoch(oankiMesaj.date.millisecondsSinceEpoch);
    if (_benimMesajimMi) {
      return Padding(
        padding: EdgeInsets.symmetric(horizontal: 8.0.w, vertical: 1.2.h),
        child: Row(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.end,
                        children: [
                          Text(formatTheDate(messageDate, format: DateFormat("dd.MM.yy HH:mm")),
                              style: TextStyle(
                                  color: Colors.white,
                                  fontFamily: "OpenSans",
                                  fontStyle: FontStyle.normal,
                                  fontSize: 10.3.spByWidth),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.end,
                        children: [
                          Text(_saatDakikaGoster(oankiMesaj.date),
                              style: TextStyle(
                                  color: Colors.white,
                                  fontWeight: FontWeight.w400,
                                  fontFamily: "OpenSans",
