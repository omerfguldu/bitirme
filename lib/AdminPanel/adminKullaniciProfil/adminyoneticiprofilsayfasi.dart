...
class AdminKarsiKullaniciProfiliYonetici extends StatefulWidget {
  final Map<String, dynamic> card;

  AdminKarsiKullaniciProfiliYonetici({this.card});

  @override
  _AdminKarsiKullaniciProfiliYoneticiState createState() => _AdminKarsiKullaniciProfiliYoneticiState();
}

FirestoreDBService _firestoreDBService = locator<FirestoreDBService>();

class _AdminKarsiKullaniciProfiliYoneticiState extends State<AdminKarsiKullaniciProfiliYonetici> with SingleTickerProviderStateMixin {
  ...
  DocumentSnapshot _cardYonetici;
  @override
  void initState() {
    super.initState();
  ...
  Future<void> yoneticidetay_getir()  async {
   _cardYonetici= await FirebaseFirestore.instance.collection("yonetici").doc(widget.card['userId']).get();
  }
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          Padding(
            ...
            child: IconButton(
              icon: Icon(Icons.settings,color: Colors.white,size: 25.0.h,),
              onPressed: (){

                Navigator.push(context,
                  MaterialPageRoute(builder: (context) => Yetkilendirme(card: widget.card,cardYonetici:_cardYonetici,)),
                );
              },
            ),
          ),
